    ...
    public class BookServiceImpl implements BookService {
        ...
        public BooksResponse getBooksByProfileId(Long profileId) {

            List<BookProfile> bookProfiles = bookProfileRepository.findByProfile_ProfileId(profileId);

            List<BooksResponse.Item> items = bookProfiles.stream().map(bookProfile -> {
                ...
                imageLinks.setSmallThumbnail(book.getImageUrl());
        ...
        public BookWithReviewsResponse getBookWithReviews(String volumeId) {
            ...
            for (BookReview review : reviewEntities) {
                ...
                response.setProfileId(review.getProfile().getProfileId());
            ...
            topLevelReviews.sort((r1, r2) -> r2.getCreatedAt().compareTo(r1.getCreatedAt()));

            for (ReviewResponse parent : topLevelReviews) {
                if (parent.getReplies() != null) {
                    parent.getReplies().sort(Comparator.comparing(ReviewResponse::getCreatedAt));
                }
            }
        ...
        public ReviewResponse addReview(ReviewRequest request) {
            ...
            if(request.getParentReviewId() != null) {
                BookReview parent = bookReviewRepository.findById(request.getParentReviewId())
                        .orElseThrow(() -> new RuntimeException("Parent review not found"));
                response.setParentReviewId(parent.getBookReviewId());
            }
